...
function dataUrl(ticker) {
  ...
  const base =
    document.body.dataset.dataBase || "../data";
...
function getViewRows(data) {
  ...
  const idx =
    pullDates.map(function (d) {

      return fullDates.indexOf(d);
...
function renderTable() {
  ...
  if (!view) {
    ...
    host.innerHTML =
      '<p class="status">' +
      "لا بيانات لهذا الانتهاء / الأيام" +
      "</p>";
...
function exportExcel() {
  ...
  if (
    ...
    setStatus(
      "مكتبة Excel لم تُحمّل — تحقق من الإنترنت",
      "err"
    );
  ...
  wb.xlsx
    ...
    .then(function (buffer) {
      ...
      setStatus(
        "تم تنزيل ملف Excel (.xlsx) بتنسيق كامل",
        "ok"
      );
